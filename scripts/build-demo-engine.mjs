/**
 * The browser demo runs the same statistics, analysis and permission logic as
 * the server.
 *
 * Rather than maintaining a second copy by hand — which would drift the first
 * time a rating formula or a derivation rule changed — this script mechanically
 * converts the server modules from CommonJS to ES modules and writes them into
 * web/src/demo/engine/. The server remains the single source of truth.
 *
